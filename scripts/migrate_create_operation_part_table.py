"""
Migracion idempotente: crea LYNX_PM_OPERATION_PART.

Parte del flujo "Notificar operacion": permite reportar varias partes objeto
por notificacion, ligadas a la Operacion (no al Aviso ni a la Causa). A
diferencia de AVISOID (VARCHAR2 sin FK real por datos historicos sucios),
Operation.Id es un NUMBER limpio generado por Oracle, asi que aqui si se
puede usar una FK real hacia LYNX_PM_AVISO_OPERATIONS(ID).

ID generado por trigger+sequence, mismo patron que LYNX_PM_AVISO_CAUSE.

Uso:  IBERO_DB_PASS debe estar en el entorno.
      python scripts/migrate_create_operation_part_table.py
"""
import os
import oracledb
...
TABLE = "LYNX_PM_OPERATION_PART"
SEQUENCE = "SEQ_LYNX_PM_OPERATION_PART_ID"
TRIGGER = "TRG_LYNX_PM_OPERATION_PART_ID"
FK_NAME = "FK_LYNX_PM_OPERATION_PART_OP"
    ...
    if table_exists(cur, TABLE):
        ...
        cur.execute(f"""
            CREATE TABLE {TABLE} (
                ...
                CONSTRAINT {FK_NAME} FOREIGN KEY (OPERATION_ID) REFERENCES LYNX_PM_AVISO_OPERATIONS(ID)
            )
        """)
        conn.commit()
